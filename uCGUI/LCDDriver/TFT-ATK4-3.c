...
void HAL_GPIO_WritePort(GPIO_TypeDef* GPIOx, uint32_t PortValue)
{
	GPIOx->ODR = PortValue;
}
...
//	HAL_GPIO_WritePort(DATA_BUS,0xffff);
	LCD_GPIO.Pin = GPIO_PIN_All;
	LCD_GPIO.Pull  = GPIO_NOPULL;
	LCD_GPIO.Mode = GPIO_MODE_INPUT;
	LCD_GPIO.Speed = GPIO_SPEED_FAST;
	HAL_GPIO_Init(DATA_BUS, &LCD_GPIO);
...
void LCD_ATK_Init(void)
{
	...
	LCD_GPIO.Mode  = GPIO_MODE_OUTPUT_PP;
	LCD_GPIO.Speed = GPIO_SPEED_FAST;
	HAL_GPIO_Init(GPIOE,&LCD_GPIO);
	HAL_GPIO_WritePin(GPIOE, GPIO_PIN_All, GPIO_PIN_SET);

	//CS-PA0
	//RS-PA3
	//WR-PA4
	LCD_GPIO.Pin   = GPIO_PIN_0 | GPIO_PIN_3 | GPIO_PIN_4;
	LCD_GPIO.Pull  = GPIO_NOPULL;
	LCD_GPIO.Mode  = GPIO_MODE_OUTPUT_PP;
	LCD_GPIO.Speed = GPIO_SPEED_FAST;
	HAL_GPIO_Init(GPIOA,&LCD_GPIO);
	HAL_GPIO_WritePin(GPIOA, GPIO_PIN_0 | GPIO_PIN_3 | GPIO_PIN_4, GPIO_PIN_SET);

	//RD-PB0
	//RST-PB1
	LCD_GPIO.Pin   = GPIO_PIN_0 | GPIO_PIN_1;
	HAL_GPIO_Init(GPIOB,&LCD_GPIO);
	HAL_GPIO_WritePin(GPIOB, GPIO_PIN_0 | GPIO_PIN_1, GPIO_PIN_SET);

	//Backlight-PC8
	LCD_GPIO.Pin   = GPIO_PIN_8;
	HAL_GPIO_Init(GPIOC,&LCD_GPIO);
	HAL_GPIO_WritePin(GPIOC, GPIO_PIN_8, GPIO_PIN_SET);//backlight on
	...
	SendString("LCD ID Is:");
	UartPutHexWord(lcddev.id);
...
void LCD_L0_DrawHLine(int x0, int y,  int x1)
{
  ...
  //Kitronix320x240x16_SSD2119LineDrawH(x0, x1, y, LCD_COLORINDEX);
...
//    Kitronix320x240x16_SSD2119LineDrawV(x, y0, y1,LCD_COLORINDEX);
	u16 index;

	for(index = y0; index <= y1; index++)
	{
		LCD_SetCursor(x,index);	//设置光标位置
		LCD_WriteRAM_Prepare();     		//开始写入GRAM
		RS_1;
		WR_0;
		WRITE_BUS(LCD_COLORINDEX);
